...
setup()
App.run(user_loop=loop)from arduino.app_utils import *
from flask import Flask, jsonify, request, render_template
import threading
import json
import time
...
def kk1l_select():
    ...
    antenna = config.get("antennas", {}).get(str(port))
    if not antenna:
                    return jsonify({"ok": False, "error": "No antenna assigned to port " + str(port)}), 409
                this_key = "input1_port" if input_n == "1" else "input2_port"
    other_key = "input2_port" if input_n == "1" else "input1_port"
    if config.get(other_key) == port:
                    return jsonify({"ok": False, "error": "Interlock - port in use by other input"}), 409
                current = config.get(this_key)
    if current == port:
                    bridge_call("kk1l_deselect", port)
                    config[this_key] = None
...
def kk1l_setband():
    ...
    port = config.get("band_map", {}).get(band)
    if port is None:
                    return jsonify({"ok": False, "error": "No port assigned for band " + band}), 404
                port = int(port)
    antenna = config.get("antennas", {}).get(str(port))
    if not antenna:
                    return jsonify({"ok": False, "error": "No antenna on port " + str(port)}), 409
...
def setband():
    ...
    relay = config["band_map"].get(band)
    if relay is None:
                    return jsonify({"ok": False, "error": "No antenna assigned for " + band}), 404
                relay = int(relay)
    other = "input2_relay" if input_n == "1" else "input1_relay"
    this = "input1_relay" if input_n == "1" else "input2_relay"
    if config[other] == relay:
