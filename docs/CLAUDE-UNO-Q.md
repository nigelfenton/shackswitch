...
├── CLAUDE.md                — legacy R4 context (historical)
├── README.md                — project overview
├── CHANGELOG.md             — version history
├── shackswitch-v2/          — current source files (reference copies)
│   ├── main.py              — Flask REST API + smartsdr launcher
│   ├── index.html           — web UI
│   └── sketch.ino           — STM32 firmware
├── services/                — systemd service files
│   ├── shackswitch.service  — systemd service definition
│   └── shackswitch-boot.sh  — boot script (OpenOCD flash + container start)
├── firmware/                — legacy R4 firmware (historical)
└── nodered/                 — legacy Pi Node-RED files (historical)
```

Live files on Uno Q at `/home/arduino/ArduinoApps/shackswitch/`
...
STM32 firmware (Uno Q MCU — Zephyr OS)
  provides relay_on/off, get_status, kk1l_* methods
    │ GPIO D2-D5 (3.3V logic) + I2C MCP23017 (KK1L)
    ▼
G0JKN relay shield (NPN/PNP, 12V coils) + KK1L 2x6 (planned)
    │
...
- Only active ports shown (controlled by port_count setting)
...
- **App name:** shackswitch (not first-app — that is legacy)
...
| Two App Lab apps exist | Use `shackswitch` not `first-app` |
...
| Near term | KK1L board build and wiring |
| Near term | MCP23017 I2C wiring and test |
| Near term | Wire kk1l_* endpoints into web UI |
| Near term | smartsdr.py — route to kk1l/setband when KK1L available |
...
| 01 Apr 2026 | App renamed from first-app to shackswitch in App Lab |
