...
EasyNextionLibrary (install via Library Manager)
...
The WiFi, RTC, EEPROM, LED Matrix and ArduinoGraphics libraries are all included with the R4 board package
...
Connect the Arduino Uno R4 WiFi to your PC via USB
Open both `shackswitch.ino` and `triggers.ino` in the Arduino IDE — they must be in the same folder
Select Tools → Board → Arduino Uno R4 WiFi
Select the correct COM port under Tools → Port
Click Upload
Uploading the Nextion HMI File
Copy the `.HMI` file to a microSD card (FAT32 formatted)
Insert the card into the Nextion display
Power the display — it will flash the new firmware automatically
Remove the card when complete
---
Wiring
Arduino to Nextion
...
For automatic antenna switching based on your radio's frequency, the SmartSwitch can be integrated with Node-RED running on a Raspberry Pi.
What You Need
Raspberry Pi (any model — a Pi Zero 2W works perfectly)
Node-RED installed on the Pi
node-red-contrib-flexradio nodes for FlexRadio SmartSDR integration
How It Works
```
FlexRadio SmartSDR
    │ TCP/IP API (network)
...
Via the web settings page — browse to `/settings`, type the new name (up to 22 characters) and click Save. The Nextion display updates immediately.
Via factory defaults — edit the `loadConfig()` function in `shackswitch.ino` before uploading.
---
Factory Reset
