...
- Add toggleable voice mode — minimal UI, high contrast, large text, ARIA labels
...
- Spoken confirmation before or after acting
...
| Event | Announcement |
|---|---|
| Antenna selected | "Input 1 now on Antenna 3" |
| Band change (SmartSDR) | "Band change — 40 metres. Antenna 2 selected." |
| Interlock blocked | "Blocked — Input 2 already using that antenna" |
| Amplifier to operate | "Amplifier in operate" |
| Amplifier to standby | "Amplifier in standby" |
| Amplifier fault | "Amplifier fault — check RF2K-S" |
| High SWR | "Warning — high SWR" |
| Status request | "Input 1: Antenna 2, 40 metres. Input 2: Antenna 4, 20 metres. Amplifier in operate." |

---

### Hardware Notes

- AirPods (Apple) — work with iPhone/iPad/Mac browser, mic suitable for voice commands
- Cheap BT earpiece with mic — works for Phase 1/2 paired to browser device
- Phase 3 BT direct to Uno Q — any BT A2DP device; mic support via HFP profile (more complex)
- Uno Q has no audio output jack — BT is the only practical audio path from the board itself

---

### UI Considerations for Voice Mode

- Toggle button to enter/exit voice mode (keyboard accessible)
- Voice mode: minimal layout, large text, high contrast
- Full ARIA labels on all buttons and status elements for screen reader compatibility
